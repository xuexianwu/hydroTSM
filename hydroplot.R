...
# new arguments: y, y.monthly, y.annual
# 'col' now has two arguments

.hydroplotts <- function(x, x.monthly, x.annual, 
                         y=NULL, y.monthly=NULL, y.annual=NULL, 
			 ...
			 pfreq="dma", tick.tstep= "auto", lab.tstep= "auto", lab.fmt=NULL,
                         ...
                         cex.main=1.3, cex.lab=1.3, cex.axis=1.3, col=c("blue","black"), 
                         lwd=1, lty=1, ...) {
      ...
      if ( pfreq == "o") {
          ...
          plot.xts(x, axes=FALSE, type="o", 
                   ...
                   cex.main=cex.main, cex.lab=cex.lab, cex.axis=cex.axis, col=col[1], 
                   lty=lty, lwd=lwd, ...)
          if (!is.null(y)) 
            lines(y, type="o",  col= col[2])#, lwd=lwd[2], lty= lty[2], pch= pch[2], cex = cex[2])  
      ...
      # Plotting the Monthly series, if needed
      if ( pfreq %in% c("dma", "dm", "ma") ) {
        ...
        plot.xts(x.monthly, axes=FALSE, type="o",
                 ...
                 cex.main=cex.main, cex.lab=cex.lab, cex.axis=cex.axis, col=col[1], 
                 lty=lty, lwd=lwd, ... )
        if (!is.null(y.monthly)) {
            message("hola")
            if ( !(is.xts(y.monthly)) ) y.monthly <- as.xts(y.monthly) 
            print(class(y.monthly))
            print(y.monthly)
            lines(y.monthly, type="o",  col= col[2])#, lwd=lwd[2], lty= lty[2], pch= pch[2], cex = cex[2]) 
        } # IF end 
        axis(2, cex.lab=cex.lab, cex.axis=cex.axis)
      ...
      if ( pfreq %in% c("dma", "ma") ) {   
          ...
          plot.xts(x.annual, axes=FALSE, type="o", 
                   ...
                   lty=lty, lwd=lwd, ...)
          if (!is.null(y.annual)) {
            if ( !(is.xts(y.annual)) ) y.annual <- as.xts(y.annual) 
            lines(y.annual, type="o",  col= col[2])#, lwd=lwd[2], lty= lty[2], pch= pch[2], cex = cex[2]) 
          } # IF end 
          axis(2, cex.lab=cex.lab, cex.axis=cex.axis)
          # Draws monthly ticks in the X axis, but labels only in years
 ...
 if ( pfreq %in% c("dma", "dm") ) {
   ...
   if (is.null(y)) {     
     boxplot( coredata(x)~years, main=title, ylab=paste(ylab," [", var.unit, "/day]", sep=""), 
       ...
       boxplot( coredata(x)~years, 
               at=0:(nyears-1)*nyears+1, xlim = c(0, (nyears-1)*nyears+3), ylim=range(x,y),
               main=title, ylab=paste(ylab," [", var.unit, "/day]", sep=""), 
               cex.main=cex.main, cex.lab=cex.lab, cex.axis=cex.axis, col=col[1], ...)
       boxplot( coredata(x)~years, 
               at=0:(nyears-1)*nyears+2, xaxt = "n", add = TRUE, col=col[2], ...)
     } # else end
